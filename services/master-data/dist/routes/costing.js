...
router.post('/calculate', async (req, res) => {
    try {
        ...
        const result = await costingService_1.costingService.calculateCost(request);
        res.json(result);
    }
    catch (error) {
...
router.get('/breakdown/:orderId', async (req, res) => {
    try {
        ...
        const breakdown = await costingService_1.costingService.getCostBreakdown(orderId);
        res.json(breakdown);
    }
    catch (error) {
 ...
 */
router.patch('/actual/:orderId', async (req, res) => {
    try {
        const { orderId } = req.params;
        ...
        await costingService_1.costingService.updateActualCost(orderId, actual_miles, actual_cost);
