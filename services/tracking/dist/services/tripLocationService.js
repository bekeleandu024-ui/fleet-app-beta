...
exports.updateRouteHistory = updateRouteHistory;
const client_1 = require("../db/client");
const geo_1 = require("../lib/geo");
const ROUTE_HISTORY_MAX_POINTS = 2000;
async function updateRouteHistory(tripId, location) {
    const timestamp = location.timestamp ? new Date(location.timestamp) : new Date();
    const client = await client_1.pool.connect();
    try {
        await client.query("BEGIN");
        const tripResult = await client.query(`SELECT id, actual_miles, route_history, last_known_lat, last_known_lng
         FROM trips
         WHERE id = $1
         FOR UPDATE`, [tripId]);
        ...
        if (previousLat !== null &&
            ...
            const distance = (0, geo_1.haversineDistanceMiles)(Number(previousLat), Number(previousLng), location.latitude, location.longitude);
            actualMiles += distance;
        }
        const routeHistory = Array.isArray(tripRow.route_history)
        ...
        await client.query(`INSERT INTO trip_locations (
          trip_id,
          driver_id,
          latitude,
          longitude,
