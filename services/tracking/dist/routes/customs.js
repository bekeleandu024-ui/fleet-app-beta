...
const CUSTOMS_PRIORITIES = ["URGENT", "HIGH", "NORMAL", "LOW"];
...
const buildTripNumber = (tripId) => {
    const normalized = typeof tripId === "string"
        ? tripId
...
router.get("/", async (_req, res) => {
    ...
    `),
            client_1.pool.query(`SELECT agent_name FROM customs_agents WHERE is_active = true ORDER BY agent_name`),
        ]);
        ...
        const stats = {
            ...
            underReview: clearances.filter((c) => c.status === "UNDER_REVIEW" || c.status === "DOCS_SUBMITTED").length,
            approved: clearances.filter((c) => c.status === "APPROVED").length,
            urgent: clearances.filter((c) => c.priority === "URGENT").length,
        };
        const crossingPointSet = new Set();
...
router.get("/:id", async (req, res) => {
    ...
    `, [id]);
        ...
        const requiredDocs = Array.isArray(clearance.required_documents)
            ? clearance.required_documents.map((doc) => String(doc).toUpperCase())
            : [];
        const allowedDocumentStatuses = ["UPLOADED", "VERIFIED", "REJECTED", "EXPIRED"];
        const normalizeDocumentStatus = (value) => {
            const status = typeof value === "string" ? value.toUpperCase() : "UPLOADED";
            return allowedDocumentStatuses.includes(status)
                ? status
                : "UPLOADED";
        };
