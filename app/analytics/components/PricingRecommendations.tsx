...
export function PricingRecommendations() {
  return (
    <Card className="bg-gray-800/30 border-gray-700/50">
      <CardHeader>
        <CardTitle className="text-white">Pricing Recommendations</CardTitle>
      ...
      <CardContent className="space-y-3">
        {mockPricingRecommendations.map((rec) => {
          ...
          return (
            <div
              key={rec.orderId}
              className={`bg-gray-700/30 p-4 rounded-lg border ${config.border} hover:border-cyan-500/50 transition-colors`}
            >
              <div className="flex items-start justify-between mb-3">
                <div className="flex-1">
                  <div className="flex items-center gap-2 mb-1">
                    <span className="text-white font-semibold">{rec.orderId}</span>
                    <Badge className={`${config.bg} ${config.color} border-0 text-xs`}>
                      <StatusIcon className="h-3 w-3 mr-1" />
                      {config.label}
                    </Badge>
                  </div>
                  <p className="text-sm text-gray-400">{rec.customer}</p>
                  <p className="text-xs text-gray-500">{rec.lane}</p>
                </div>
              ...
              <div className="grid grid-cols-2 gap-3 mb-3">
                <div className="bg-gray-800/50 p-2 rounded">
                  ...
                  <p className="text-lg font-semibold text-white">${rec.currentPrice.toFixed(2)}</p>
                </div>
                <div className="bg-gray-800/50 p-2 rounded border border-cyan-700/50">
