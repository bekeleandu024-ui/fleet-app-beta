    ...
    <>
          ...
          {kpis.map((kpi) => (
            ...
            >
              ...
              <div className="min-w-0">
                ...
                <p className="text-lg font-semibold text-[var(--text)]">{kpi.value}</p>
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="flex flex-wrap items-center gap-2 text-xs">
          {data.serviceHealth.map((service) => (
            <StatChip
              ...
              variant={service.status === "ok" ? "ok" : service.status === "warn" ? "warn" : "alert"}
            />
          ))}
        </div>
      </section>
      ...
      <section className="col-span-12 grid gap-6 lg:grid-cols-12">
        <article className="lg:col-span-7 xl:col-span-8 flex flex-col gap-4 rounded-xl border border-subtle bg-surface-1 p-4 shadow-soft">
          <header className="flex items-center justify-between">
            <div>
              ...
              <p className="text-xs text-muted">Filter the active network and monitor critical movement.</p>
            </div>
          ...
          <div className="grid gap-4 lg:grid-cols-12">
            <div className="lg:col-span-4 space-y-4">
              ...
              <div className="rounded-xl border border-subtle bg-surface-2 px-3 py-2 text-xs text-muted">
                Hotspots: <span className="font-semibold text-[var(--text)]">{data.liveNetwork.mapSummary.hotspots}</span>
                <br />
                Dwell alerts: <span className="font-semibold text-[var(--text)]">{data.liveNetwork.mapSummary.dwellAlerts}</span>
              </div>
            </div>
        ...
        <article className="lg:col-span-5 xl:col-span-4 flex flex-col gap-4 rounded-xl border border-subtle bg-surface-1 p-4 shadow-soft">
          <header>
            <h2 className="text-sm font-semibold text-[var(--text)]">Today at a Glance</h2>
            <p className="text-xs text-muted">Lane performance, available crews, and equipment readiness.</p>
          </header>
          <div className="grid gap-4 md:grid-cols-2">
            <section className="space-y-3">
              <h3 className="text-xs font-semibold uppercase tracking-wide text-muted">Top Lanes</h3>
              <ul className="space-y-2 text-sm">
                {data.glance.topLanes.map((lane) => (
                  <li key={lane.lane} className="flex items-center justify-between text-[var(--text)]">
                    <span>{lane.lane}</span>
                    <span className="text-xs text-muted">{lane.orders} loads • {formatPercent(lane.onTimePercent)}</span>
                  </li>
                ))}
              </ul>
            </section>
      ...
      <section className="col-span-12">
        <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-4">
          {Array.from({ length: 4 }).map((_, index) => (
            <div key={index} className="h-20 animate-pulse rounded-xl border border-subtle bg-surface-2" />
          ))}
        </div>
      </section>
      <section className="col-span-12">
        <div className="flex gap-2">
          {Array.from({ length: 3 }).map((_, index) => (
            <div key={index} className="h-6 w-32 animate-pulse rounded-xl bg-surface-2" />
          ))}
        </div>
      </section>
      <section className="col-span-12 grid gap-6 lg:grid-cols-12">
