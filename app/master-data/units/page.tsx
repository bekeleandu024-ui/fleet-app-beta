...
export default function UnitsMasterDataPage() {
  ...
  const columns: DataTableColumn<MasterDataResponse["data"][number]>[] = [
    ...
    { key: "name", header: "Identifier", accessor: (row) => row.name, widthClass: "min-w-[140px]" },
  ...
  return (
    <>
              ...
              <div className="grid gap-2">
                {data.filters.statuses.map((status) => (
                  <label key={status} className="flex items-center gap-2 text-sm text-[var(--text)]">
                    <input type="checkbox" className="size-3 accent-[var(--brand)]" defaultChecked={status === "Available"} />
                    <span>{status}</span>
                  </label>
                ))}
              </div>
      ...
      <div className="col-span-12 flex flex-col gap-6 lg:col-span-9">
        <Toolbar
          title="Units"
          description="Equipment inventory across the network. Editing is disabled."
        />
