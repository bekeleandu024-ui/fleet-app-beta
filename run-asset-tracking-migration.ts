...
async function runMigration() {
  try {
    ...
    `);
    console.log(`✓ Unit profiles columns added: ${unitsCheck.rows.map((r: any) => r.column_name).join(', ')}`);
    ...
    const trailersCheck = await pool.query(`
      ...
      ORDER BY column_name
    `);
    console.log(`✓ Trailers columns added: ${trailersCheck.rows.map((r: any) => r.column_name).join(', ')}`);
    ...
    const ordersCheck = await pool.query(`
      ...
      ORDER BY column_name
    `);
    console.log(`✓ Orders columns added: ${ordersCheck.rows.map((r: any) => r.column_name).join(', ')}`);
    ...
    const tripsCheck = await pool.query(`
      ...
      ORDER BY column_name
    `);
    console.log(`✓ Trips columns added: ${tripsCheck.rows.map((r: any) => r.column_name).join(', ')}`);
