const { Pool } = require('pg');

const pool = new Pool({
  connectionString: process.env.DATABASE_URL || 'postgresql://postgres:postgres@localhost:5432/fleet',
});

async function testDashboardQuery() {
  try {
    // This is the exact query from the dashboard metrics API
    const query = `
      WITH 
      order_stats AS (
        SELECT 
          COUNT(*) FILTER (WHERE status IN ('New', 'Planning', 'Pending')) as waiting_count
        FROM orders
      ),
      trip_stats AS (
        SELECT 
          COUNT(*) FILTER (WHERE status IN ('in_transit', 'en_route_to_pickup', 'departed_pickup') AND risk_level IN ('High', 'Critical')) as risk_count,
          COUNT(*) FILTER (WHERE status = 'completed' AND completed_at > NOW() - INTERVAL '30 days') as completed_30d,
          COUNT(*) FILTER (WHERE status = 'completed' AND completed_at > NOW() - INTERVAL '30 days' AND on_time_delivery = true) as on_time_30d
        FROM trips
      ),
      driver_stats AS (
        SELECT 
          COUNT(*) as total_drivers,
          COUNT(*) FILTER (WHERE is_active = true) as active_drivers
        FROM driver_profiles
      ),
      financial_stats AS (
        SELECT 
          COALESCE(SUM(revenue), 0) as total_revenue,
          COALESCE(SUM(total_cost), 0) as total_cost
        FROM trip_costs
        WHERE created_at > NOW() - INTERVAL '30 days'
      )
      SELECT 
        o.waiting_count,
        t.risk_count,
        t.completed_30d,
        t.on_time_30d,
        d.total_drivers,
        d.active_drivers,
        f.total_revenue,
        f.total_cost
      FROM order_stats o, trip_stats t, driver_stats d, financial_stats f
    `;

    const result = await pool.query(query);
    const row = result.rows[0];

    const totalRevenue = parseFloat(row.total_revenue);
    const totalCost = parseFloat(row.total_cost);
    const netMargin = totalRevenue - totalCost;
    const avgMargin = totalRevenue > 0 ? (netMargin / totalRevenue) * 100 : 0;

    const onTimePercent = parseInt(row.completed_30d) > 0 
      ? (parseInt(row.on_time_30d) / parseInt(row.completed_30d)) * 100 
      : 0;

    console.log('\n=== DASHBOARD METRICS (From Database) ===');
    console.log(`Completed Trips (30d): ${row.completed_30d}`);
    console.log(`On-Time Deliveries: ${row.on_time_30d}`);
    console.log(`On-Time Percentage: ${onTimePercent.toFixed(1)}%`);
    console.log(`\nOrders Waiting: ${row.waiting_count}`);
    console.log(`At-Risk Trips: ${row.risk_count}`);
    console.log(`Active Drivers: ${row.active_drivers} / ${row.total_drivers}`);
    console.log(`Total Revenue: $${totalRevenue.toFixed(2)}`);
    console.log(`Total Cost: $${totalCost.toFixed(2)}`);
    console.log(`Net Margin: $${netMargin.toFixed(2)}`);
    console.log(`Avg Margin %: ${avgMargin.toFixed(1)}%`);
    
  } catch (error) {
    console.error('Error:', error.message);
  } finally {
    await pool.end();
  }
}

testDashboardQuery();
